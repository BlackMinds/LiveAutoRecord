<template>
    <v-card>
        ...
        <v-card-actions class="border-t justify-end">
            <v-btn v-if="recorder.state === 'idle'" @click="startRecord" :loading="requesting" size="small">
                刷新
            </v-btn>
            <v-btn v-else @click="stopRecord" :loading="requesting" :disabled="recorder.state === 'stopping-record'" size="small">
                终止
            </v-btn>
...
console.error('recorderr', recorder)
...
let time = localStorage.getItem('automaticSegmentation')
