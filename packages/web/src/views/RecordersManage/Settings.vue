<template>
    <v-card class="mx-auto my-4 max-w-2xl">
        ...
        <v-card-item>
            ...
            <v-form v-else>
                <v-text-field label="保存路径规则" v-model="manager.savePathRule" append-inner-icon="mdi-help-circle" @click:append-inner="
          ...
          " />
                ...
                <v-text-field v-if="manager.autoCheckLiveStatusAndRecord" label="检查间隔（毫秒）" v-model="manager.autoCheckInterval" type="number" />

                <v-text-field label="自动分段间隔(毫秒)" v-model="manager.automaticSegmentation" type="number" />
            </v-form>
        </v-card-item>
