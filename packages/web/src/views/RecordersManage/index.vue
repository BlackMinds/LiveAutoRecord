        ...
        <div class="h-screen overflow-auto" :class="{ hidden: currentRouteParentIsRecordersManage }">
            <div class="sticky left-0 top-0 p-4 bg-inherit shadow flex">
                <div class="flex flex-auto gap-4">
                    <v-select label="排序" class="basis-64 shrink-0 grow-0" v-model="sortMode" :items="sortModes" item-title="name" :item-value="(item) => item" hide-details="auto" />
            ...
            <div class="flex flex-wrap gap-4 p-4">
                <RecorderCard v-for="recorder in displayingRecorders" :key="recorder.id" :recorder="recorder" class="flex-shrink-0 flex-grow basis-48" />
            </div>
        </div>
...
console.error('displayingRecorders', displayingRecorders)
